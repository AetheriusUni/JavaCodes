class Knapsack {
    ...
    public static void main(String[] args) throws Exception 
    {
        ...
        int val[] = {10, 40, 30, 50};	// value of stuff
        int wt[] = {5, 4, 6, 3};		// weight of that stuff
        int W = 10;						// weight thief can carry 
    ...
    public static int knapsack(int val[], int wt[], int W) 
    {
        ...
        int[][] V = new int[N + 1][W + 1]; //Create a matrix. Items are in rows and weight at in columns, +1 on each side to add the 0 row/column
        ...
        for (int item=1;item<=N;item++)
        {
            ...
            for (int weight=1;weight<=W;weight++){
                ...
                if (wt[item-1]<=weight)
                {
                	...
                	// current item is val[item-1]
                	// V[item-1][weight-wt[item-1]] is value[previous row][current weight limit minus current item = weight left in bag]
        ...
        return V[N][W];	// max value you can get in that knapsack
    }
}